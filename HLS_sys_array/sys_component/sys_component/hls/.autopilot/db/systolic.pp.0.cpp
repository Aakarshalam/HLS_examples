...
__attribute__((sdx_kernel("systolic_array_kernel", 0))) void systolic_array_kernel(
    data_t A[8][8],
    data_t B[8][8],
    data_t C[8][8]
);
...
#line 1 "directive"
#pragma HLSDIRECTIVE TOP name=systolic_array_kernel
# 15 "systolic.cpp"
...
#pragma HLS ARRAY_PARTITION variable=b_data complete dim=0
 ...
 INIT_PE: for (int i = 0; i < 8; i++) {
        VITIS_LOOP_41_1: for (int j = 0; j < 8; j++) {
#pragma HLS UNROLL
 pe_array[i][j].reset();
        }
    }

    INIT_A: for (int i = 0; i < 8; i++) {
...
#pragma HLS ARRAY_PARTITION variable=b_temp complete dim=0
 ...
 INIT_A_TEMP: for (int i = 0; i < 8; i++) {
            VITIS_LOOP_113_4: for (int j = 0; j <= 8; j++) {
#pragma HLS UNROLL
 a_temp[i][j] = 0;
            }
        }

        INIT_B_TEMP: for (int i = 0; i <= 8; i++) {
            VITIS_LOOP_120_5: for (int j = 0; j < 8; j++) {
#pragma HLS UNROLL
 b_temp[i][j] = 0;
            }
        }
        ...
        COMPUTE_I: for (int i = 0; i < 8; i++) {
            COMPUTE_J: for (int j = 0; j < 8; j++) {
#pragma HLS UNROLL
 ...
 pe_array[i][j].compute(
        ...
        FORWARD_A: for (int i = 0; i < 8; i++) {
            VITIS_LOOP_142_6: for (int j = 1; j <= 8; j++) {
#pragma HLS UNROLL
 a_data[i][j] = a_temp[i][j];
            }
        }

        FORWARD_B: for (int i = 1; i <= 8; i++) {
            VITIS_LOOP_149_7: for (int j = 0; j < 8; j++) {
#pragma HLS UNROLL
 b_data[i][j] = b_temp[i][j];
    ...
    EXTRACT_I: for (int i = 0; i < 8; i++) {
        EXTRACT_J: for (int j = 0; j < 8; j++) {
#pragma HLS PIPELINE II=1
