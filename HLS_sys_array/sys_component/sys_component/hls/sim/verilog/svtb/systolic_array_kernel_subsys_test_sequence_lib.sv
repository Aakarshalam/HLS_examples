...
//
//==============================================================
`ifndef SYSTOLIC_ARRAY_KERNEL_SUBSYS_TEST_SEQUENCE_LIB__SV                                              
    `define SYSTOLIC_ARRAY_KERNEL_SUBSYS_TEST_SEQUENCE_LIB__SV                                          
                                                                                                    
                                                                                                    
    `include "uvm_macros.svh"                                                                     
                                                                                                    
    class systolic_array_kernel_subsys_test_sequence_lib extends uvm_sequence;                                
                                                                                                    
        function new (string name = "systolic_array_kernel_subsys_test_sequence_lib");                      
            super.new(name);                                                                        
            `uvm_info(this.get_full_name(), "new is called", UVM_LOW)                             
        endfunction                                                                                 
                                                                                                    
        `uvm_object_utils(systolic_array_kernel_subsys_test_sequence_lib)                                     
        `uvm_declare_p_sequencer(systolic_array_kernel_virtual_sequencer)                                     
        ...
        virtual task body();                                                                        
            ...
            if (!uvm_config_db#(systolic_array_kernel_reference_model)::get(p_sequencer,"", "refm", refm))
                        ...
                        begin
                            ...
                            for(int i=0; i<5; i++) begin
                                ...
                                for(int j=0; j < (64+32-1)/32; j++) begin
                                    data64bit_A.push_back( ((refm.mem_blk_pages_gmem0.maxi_bundlevar_offset["A"]+refm.mem_blk_pages_gmem0.page_ofst[refm.mem_blk_pages_gmem0.rd_page_idx])>>(j*32)) & (2**32-1) );
                                end
